...
export async function POST(request: NextRequest) {
  try {
    ...
    // Создаём заявку
    // Примечание: order_requests не имеет client_id, только контактные данные
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (my === 'true' && userId) {
      // Фильтруем по client_id (связь с пользователем)
    ...
    // priority_score не существует в схеме, используем только created_at
