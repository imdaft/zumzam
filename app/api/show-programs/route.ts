...
export async function GET(request: NextRequest) {
  try {
    ...
    const programs = await prisma.show_programs.findMany({
      where: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const program = await prisma.show_programs.create({
      data: body
    })

    logger.info('[Show Programs API] Created:', program.id)
    return NextResponse.json({ program }, { status: 201 })
  } catch (error: any) {
    logger.error('[Show Programs API] POST error:', error)
    return NextResponse.json({ error: error.message }, { status: 500 })
  }
}
