...
import { SHOW_GENRES, SHOW_DURATIONS, SHOW_AGE_RANGES, type ShowGenre, type ShowAgeRange } from '@/lib/constants/show-genres'
...
const formSchema = z.object({
  ...
  duration: z.number({ required_error: 'Укажите длительность', invalid_type_error: 'Укажите длительность' }).min(1, 'Укажите длительность'),
  performers_count: z.number({ required_error: 'Укажите количество артистов', invalid_type_error: 'Укажите количество артистов' }).min(1, 'Минимум 1 артист'),
  age_ranges: z.array(z.string()).default([]),
  requires_sound: z.boolean().default(false),
  stage_requirements: z.string().optional(),
  requires_light: z.boolean().default(false),
  additional_requirements: z.string().optional(),
})

type FormValues = z.infer<typeof formSchema>

interface ShowProgramFormProps {
  profileId: string
  initialData?: any
  isEditMode?: boolean
  onSuccess?: () => void
...
export function ShowProgramForm({
  profileId,
  initialData,
  isEditMode = false,
  onSuccess,
  onCancel,
}: ShowProgramFormProps) {
  ...
  useEffect(() => {
    ...
    const loadProfileMeta = async () => {
      try {
        ...
        console.warn('[ShowProgramForm] Failed to load profile meta:', e)
      }
    }
    loadProfileMeta()
    return () => {
  ...
  const defaultValues = useMemo<FormValues>(() => {
    ...
    return {
      ...
      performers_count: initialData.performers_count || 0,
      age_ranges: Array.isArray(initialData.age_ranges) ? initialData.age_ranges : [],
      requires_sound: initialData.requires_sound || false,
      stage_requirements: initialData.stage_requirements || '',
      requires_light: initialData.requires_light || false,
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      const url = isEditMode
        ? `/api/show-programs/${initialData.id}`
        : '/api/show-programs'
      ...
      toast.success(isEditMode ? 'Программа обновлена' : 'Программа добавлена')
      if (onSuccess) {
        onSuccess()
      }
    } catch (error) {
      ...
      toast.error(`Ошибка: ${error instanceof Error ? error.message : 'Не удалось сохранить программу'}`)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handlePhotoCropComplete = async (
    ...
    try {
      ...
      const croppedFile = new File([blob], `show-${Date.now()}.jpg`, { type: 'image/jpeg' })
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 max-w-3xl mx-auto pb-20">
        <div className="flex items-center gap-4 mb-6">
          ...
          <h1 className="text-2xl font-bold text-slate-900">
            {isEditMode ? 'Редактировать программу' : 'Добавить программу'}
          </h1>
        </div>
        ...
        <Card className="border-none shadow-[0_2px_8px_rgba(0,0,0,0.06)] rounded-[32px]">
          ...
          <CardContent className="space-y-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-base font-semibold">Название программы *</FormLabel>
                  <FormControl>
                    <Input
                      placeholder="Например: Шоу мыльных пузырей, Огненное шоу"
              ...
              render={({ field }) => (
                <FormItem>
                  <AIFieldAssistantWrapper
                    ...
                    profileName={(profileMeta?.display_name || programName || 'Шоу-программа').trim()}
                    category={profileMeta?.category || 'show'}
                    subtype={undefined}
                    existingData={{
                  ...
                  >
                    ...
                    <FormControl>
                      <Textarea
                        placeholder="Расскажите, что включает программа, какие номера, спецэффекты..."
                        className="min-h-[120px] rounded-[18px]"
                        {...field}
                      />
                    </FormControl>
                    <FormDescription>
                      Опишите, что будет происходить во время выступления, какие эффекты используются
                    </FormDescription>
                    <FormMessage />
                  </AIFieldAssistantWrapper>
                </FormItem>
        ...
        <Card className="border-none shadow-[0_2px_8px_rgba(0,0,0,0.06)] rounded-[32px]">
          ...
          <CardContent className="space-y-4">
            ...
            <Button
              type="button"
              variant="outline"
              onClick={() => document.getElementById('show-photo-upload')?.click()}
              disabled={uploading}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Добавьте ссылку на YouTube, Rutube или VK Видео с записью выступления
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
        ...
        <Card className="border-none shadow-[0_2px_8px_rgba(0,0,0,0.06)] rounded-[32px]">
          ...
          <CardContent className="space-y-6">
            <FormField
              ...
              render={() => (
                <FormItem>
                  <FormLabel className="text-base font-semibold">Жанр шоу * (можно выбрать несколько)</FormLabel>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                    {Object.entries(SHOW_GENRES).map(([key, label]) => (
                      <FormField
                        key={key}
                        control={form.control}
                        name="genres"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-base font-semibold">Количество артистов *</FormLabel>
                    <FormControl>
                      <Input
                        type="number"
                        min={1}
            ...
            <FormField
              ...
              render={() => (
                <FormItem>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                    {Object.entries(SHOW_AGE_RANGES).map(([key, label]) => (
                      <FormField
                        key={key}
                        control={form.control}
                        name="age_ranges"
                            ...
                            <FormControl>
                              <Checkbox
                                ...
                                onCheckedChange={(checked) => {
                                  ...
                                  if (key === 'universal' && checked) {
                                    updated = ['universal']
                                  } else if (key !== 'universal' && checked) {
        ...
        <Card className="border-none shadow-[0_2px_8px_rgba(0,0,0,0.06)] rounded-[32px]">
          <CardHeader>
            <CardTitle className="text-xl font-bold">Технические требования</CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            <FormField
              control={form.control}
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-center space-x-3 space-y-0 rounded-[18px] border border-slate-200 p-4 bg-slate-50/50">
                  ...
                  <FormLabel className="text-sm font-semibold cursor-pointer">
                    Требуется звуковое оборудование
                  </FormLabel>
                </FormItem>
              )}
            />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-center space-x-3 space-y-0 rounded-[18px] border border-slate-200 p-4 bg-slate-50/50">
                  ...
                  <FormLabel className="text-sm font-semibold cursor-pointer">
                    Требуется световое оборудование
                  </FormLabel>
                </FormItem>
              )}
            />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Требования к сцене</FormLabel>
                  <FormControl>
                    <Textarea
                      placeholder="Опишите требования к размеру сцены, высоте потолков и т.д."
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      placeholder="Другие важные требования для проведения шоу"
                      className="min-h-[80px] rounded-[18px]"
                      {...field}
                    />
                  </FormControl>
        ...
        <div className="flex items-center justify-end gap-4 pt-6 border-t">
          ...
          >
            {isLoading ? (
              ...
              <><CheckCircle2 className="mr-2 h-4 w-4" /> {isEditMode ? 'Сохранить изменения' : 'Добавить программу'}</>
            )}
          </Button>
        </div>
      </form>
